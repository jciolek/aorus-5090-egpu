...
parse_shell_array_words() {
    ...
    printf -v "$output_var" '%s' "$(declare -p parsed)"
}
...
rewrite_mkinitcpio() {
    local tmp_file line raw token filtered=() parsed_decl
    local -a modules=()
    tmp_file="$(mktemp)"
    ...
    while IFS= read -r line || [[ -n "$line" ]]; do
        if [[ "$line" =~ ^MODULES=\((.*)\)$ ]]; then
            raw="${BASH_REMATCH[1]}"
            parse_shell_array_words "$raw" parsed_decl
            eval "${parsed_decl/parsed/modules}"
            filtered=()
            ...
            printf 'MODULES=(' >>"$tmp_file"
            local first=1
            for token in "${filtered[@]}"; do
                local rendered_token
                printf -v rendered_token '%q' "$token"
                if [[ "$first" -eq 0 ]]; then
                    printf ' ' >>"$tmp_file"
                fi
                printf '%s' "$rendered_token" >>"$tmp_file"
                first=0
            done
            printf ')\n' >>"$tmp_file"
        else
